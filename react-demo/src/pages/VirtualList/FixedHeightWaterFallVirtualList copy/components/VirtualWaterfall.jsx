...
const VirtualWaterfall = (props) => {
  ...
  return (
    <div className="VirtualWaterfall" ref={containerRef} onScroll={handleScroll}>
      <div className="VirtualWaterfall-list" style={listStyle}>
        {renderList.map(({ item, style }) => (
          <div className="VirtualWaterfall-item" key={item.id} style={style}>
            {props.children(item)}
          </div>
        ))}
      </div>
